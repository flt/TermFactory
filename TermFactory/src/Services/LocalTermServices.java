...
public class LocalTermServices {
	...
	public int getParentClass(String collectionName){
		FindIterable<Document> classes = mongoClient.iterateDocument(collectionName,"{\"parentList\":null}");
		boolean flag = false;
		try{
	...
	public void getParentClass(LocalTerm lt){
		List<LocalTerm> relatedLocalTerms;
		if(lt.getSource() == null || lt.getSource().size() == 0)
			relatedLocalTerms = getSingleSourceInfo(lt);
		else
			relatedLocalTerms = Arrays.asList(lt);
		for(LocalTerm ltItem : relatedLocalTerms){
			List<LocalTerm> subList = bioPortalDao.getParentClass(ltItem);
			if(subList == null)
				return;
			List<String> subStringList = new ArrayList<String>();
			//根据source信息去查找ID,然后增加
